                ...
                <div class="col-lg-12">
                    ...
                    <div v-else class="swiper recent-launch-car-slider" ref="swiperContainer">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide" v-for="car in cars" :key="car.car_id">
                                <div class="product-card2">
                                    <div class="product-img">
                                        ...
                                        <a :href="`/details/${car.car_id}`">
                                            <img :src="getPrimaryImage(car)" :alt="getCarName(car)" />
                                        </a>
                                    </div>
                                    <div class="product-content">
                                        ...
                                        <div class="price">
                                            <strong>{{ getCarPrice(car) }}</strong>
                                        </div>
                                        <h6>
                                            <a :href="`/details/${car.car_id}`">{{ getCarName(car) }}</a>
                                        </h6>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    async mounted() {
        await this.fetchLatestCars();
        // Initialiser Swiper après le chargement des données
        this.$nextTick(() => {
            this.initSwiper();
        });
    },
    ...
    methods: {
        async fetchLatestCars() {
            try {
                this.loading = true;
                this.error = null;

                const response = await axios.get('http://127.0.0.1:8000/api/en/cars/latest');
                ...
                this.error = 'Erreur lors du chargement des voitures. Veuillez réessayer plus tard.';
        ...
        initSwiper() {
            ...
            if (swiperContainer && this.cars.length > 0) {
                this.swiper = new Swiper(swiperContainer, {
                    slidesPerView: 1,
                    spaceBetween: 25,
                    loop: this.cars.length > 3,
                    autoplay: {
                        delay: 4000,
        ...
        getPrimaryImage(car) {
            ...
            return primary ? primary.image_url : 'assets/img/placeholder-car.jpg';
        },
        ...
        getCarPrice(car) {
            const price = car.promotion?.current_price || car.price;
